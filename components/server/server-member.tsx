...
export const ServerMember = ({member, server}: ServerMemberProps) => {
    ...
    const onClick = () => {
        ...
        localStorage.setItem(`last-visited-${params?.serverId}`, url)
        router.push(url)
    }

    const onHover = () => {
        router.prefetch(`/servers/${params?.serverId}/conversations/${member.id}`)
    }

    return (
        <button onMouseEnter={onHover} onClick={onClick} className={cn("group px-2 py-2 rounded-md flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition mb-1", params?.memberId === member.id && "bg-zinc-700/20 dark:bg-zinc-700")}>
